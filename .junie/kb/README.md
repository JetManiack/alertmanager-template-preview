...
- **Autocomplete**: Implemented code autocompletion for Alertmanager templates in CodeMirror 6.
    ...
    - **Multi-Mode Support**: The application now supports two separate modes: **Alertmanager** and **Prometheus**.
        ...
        - **Query Mocking**: Since there is no live Prometheus server, the `query(string)` function is supported via mocking. 
            - **Data Structure**: Add a `queries` key to the "Alert Data" YAML/JSON. Each sub-key is the PromQL query string, and the value is a list of samples.
            - **Sample Structure**: Each sample should have a `metric` (labels) and a `value` field.
            - **Example**:
              ```yaml
              queries:
                "up":
                  - metric: {instance: "localhost"}
                    value: 1
              ```
            - **Helper Functions**: The renderer also supports `first(vector)`, `last(vector)`, `value(sample)`, and `label(string, sample)` to work with these mocked results.
